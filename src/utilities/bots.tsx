...
export const createBot = async ({
  ...
  console.log(
    'BEFORE botNameValidation IF statement - bot in bot creation with valid bot name !!!!!!!!: ',
    currentBot
  )
  ...
  if (botNameValidation) {
    ...
    console.log(
      'bot in bot creation with valid bot name !!!!!!!!: ',
      currentBot
    )
    setBot({
      ...currentBot,
      ...{
        ...
        semiPermaName: currentBot.botName || 'Bot',
      },
    })
...
type SelectChoresProps = Pick<
  ExecutionerProps,
  'executionState' | 'currentScore'
> & {
  first: string[]
  second: string[]
  bot: any
...
export const selectChores = async ({
  ...
  const executeSecondChoreSet = () => {
    ...
    executionState.setWorkTasks({
      ...executionState.workTasks,
      ...{ choreList: 'Outdoor Chores' },
    })
  }

  randChoice() > 0.3 ? executeFirstChoreSet() : executeSecondChoreSet()
}
...
export const doChores = ({
  ...
  executionState.setCounters({
    ...executionState.counters,
    ...{ choreClick: executionState.counters.choreClick + 1 },
  })
  ...
  speakerHandler(38575 / 1000, '').then(() => {
    if (executionState.workTasks.taskIsComplete === false) {
...
export function drillPractice({
  ...
  femaleDefault.and(
    `${currentBot.semiPermaName || 'Bot'} activated and ready!}`
  )
  ...
  executionState.setWorkTasks({
    ...executionState.workTasks,
    ...{ workDone: executionState.workTasks.workDone + 5 },
  })
  executionState.setIsDisabled({
    isDisabledBurglar: true,
    isDisabledChore: true,
  ...
  try {
    ...
    axios
      ...
      .then((allScores) => {
        executionState.setWorkTasks({
          ...executionState.workTasks,
          ...{ workDone: executionState.counters.progressInterval },
        })
...
export const burglarDefense = ({
  ...
  executionState.setCounters({
    ...executionState.counters,
    ...{ progressInterval: executionState.counters.progressInterval + 5 },
  })
  ...
  speakerHandler(16, '').then(() => setWinner(undefined))
}
